...
> keyboard. This is the same mechanism the OneXPlayer APEX uses
> (`plugins/apex-fixes` maps a button → F16), adapted to the Deck.
...
- **OneXPlayer APEX / ROG Ally / Legion Go (Bazzite/SteamOS-other)**: InputPlumber
  ...
  `plugins/apex-fixes` already does this. Button source names differ per device.
...
`scripts/deck-overlay-trigger/`, and `scripts/setup-deck-overlay-trigger.sh`
...
```
scripts/
├── setup-deck-overlay-trigger.sh        ← entry-point (~90 lines, no heredocs)
...
This matches the shape `plugins/apex-fixes` already uses for its own IP
config (`plugins/apex-fixes/config/inputplumber/{profiles,devices}/`): real
YAMLs with `yaml-language-server` schema headers, reviewable in diffs,
editable in any LSP-aware editor. The installer is a wrapper around them.

### Future: user-configurable button via the overlay UI

1. **DMI-guarded setup** (like `apex-fixes` `onLoad`): on a Deck
   (`product_name` Galileo/Jupiter, `sys_vendor` Valve), run the setup —
   install the device override, profile, udev `uaccess` rule, boot one-shot,
   and enable the services.
2. **Button picker in `WelcomeScreen` / `ControllerShortcuts`**: L4 / L5 / R4
   / R5 / Off — *not* a `Guide+` chord (those can't work on the Deck, see
   above). Changing the selection **re-runs `render-profile.sh`** with the
   new button list and reloads via `CompositeDevice.LoadProfilePath` — no
   reboot.
3. **F16 is the wake key** (not Ctrl+4): single key, no terminal/SIGQUIT side
   effects, and matches the overlay's QAM watch + the APEX setup.
