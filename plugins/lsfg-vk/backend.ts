...
const STORE_DIR = () => join(HOME(), ".config/loadout/lsfg-vk");
 ...
 *
 * Filesystem operations inside this plugin still use `WRAPPER_PATH()`
 * (the absolute path) — only what we hand off to Steam goes via the
 * tilde token.
 */
const WRAPPER_TOKEN = "~/lsfg";
...
export default class LsfgVkBackend implements PluginBackend {
  ...
  private async _writeTomlConfig(): Promise<void> {
    ...
    await writeFile(TOML_PATH(), this._renderToml(dll));
  }
  ...
  private _renderToml(dllPath: string): string {
    ...
    // (no LSFG_PROCESS export). Clamp to 2 in TOML to keep the layer happy
    // for re-enabling later.
    const tomlMultiplier = s.multiplier > 0 ? s.multiplier : 2;
    return [
      "version = 1",
      "",
      "[global]",
      ...
      `hdr_mode = ${s.hdr_mode}`,
      `experimental_present_mode = "${s.experimental_present_mode}"`,
      "",
    ].join("\n");
  }
  ...
  private async _writeWrapperScript(): Promise<void> {
    ...
    const enabled = this.settings.multiplier > 0;
    const verbose = this.settings.verbose_logging;
    const lines = [
      "#!/bin/bash",
    ...
    await writeFile(WRAPPER_PATH(), lines.join("\n"));
  ...
  private async _loadStore(): Promise<void> {
    try {
      ...
      const data = (await file.json()) as PersistedStore;
