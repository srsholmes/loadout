...
//
// Pulls library + artwork from the `__core:game-library` core service
// (full appmanifest scan, hosted by the loader — replaces the retired
// `game-browser` plugin), the running game from `__core:game-detection`
// via `useCurrentGame`, and existing launch-options strings from the
// `launch-options` plugin. The library + launch-options paths degrade
// gracefully if the matching service / plugin is missing.
//
...
import type { LaunchOptionsEntry } from "./lib/types";
...
interface GamePickerProps {
  ...
  onCollectionsLoaded?: (collections: GameCollection[], total: number) => void;
}
...
function GameRow({
  ...
  // `__core:game-library` already emits the right URLs: `capsuleUrl`
  // is the portrait library tile (or the loader's local-grid
  // endpoint for shortcuts) and `headerUrl` is the landscape header.
  // Use them as-is so the picker stays in sync with what SGDB shows.
...
export function GamePicker({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [games, withLO, cols] = await Promise.all([
        gameLibraryBackend.call("getGames").catch(() => []) as Promise<
          GameInfo[]
        >,
        launchOptionsBackend
          .call("getGames")
          .catch(() => []) as Promise<LaunchOptionsEntry[]>,
        gameLibraryBackend.call("getCollections").catch(() => []) as Promise<
          GameCollection[]
        >,
      ]);
      // Defensive dedupe by appId — the core service dedupes too, but
      // we don't want to assume that and double-render rows on stale
      // clients.
  ...
  }, [gameLibraryBackend, launchOptionsBackend, onCollectionsLoaded]);
