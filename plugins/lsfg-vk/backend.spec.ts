...
import type LsfgVkBackend from "./backend";

// The backend hard-codes paths from homedir(). For tests we redirect HOME
// to a tmpdir before importing the module so all writes land in a sandbox
// we can inspect and clean up.

let sandboxHome: string;
const originalHome = process.env.HOME;

async function freshBackend(): Promise<LsfgVkBackend> {
  // Re-import so the module-scope `homedir()` capture re-evaluates.
...
describe("LsfgVkBackend", () => {
  ...
  describe("TOML output", () => {
    it("renders a valid conf.toml after settings change", async () => {
      ...
      // Trigger config write by calling the private writer through cast.
      // (updateSettings only writes when the .so exists; we test the
      ...
      expect(toml).toContain('current_profile = "loadout-lsfg-vk"');
      expect(toml).toContain('exe = "loadout-lsfg-vk"');
    ...
    it("escapes backslashes and quotes in DLL path", async () => {
      const weirdDir = join(sandboxHome, 'has "quote" and\\backslash');
      const weirdPath = join(weirdDir, "Lossless.dll");
      await mkdir(weirdDir, { recursive: true });
      await writeFile(weirdPath, "PE...");
      await backend.setCustomDllPath(weirdPath);
      await (
        backend as unknown as { _writeTomlConfig: () => Promise<void> }
      )._writeTomlConfig();
  ...
  describe("wrapper script", () => {
    it("writes ~/lsfg with shebang, LSFG_PROCESS export, and exec $@", async () => {
      ...
      expect(script).toContain("export LSFG_PROCESS=loadout-lsfg-vk");
    ...
    it("verbose_logging adds LSFG_LOG + VK_LOADER_DEBUG to wrapper", async () => {
      await backend.updateSettings({ verbose_logging: true });
      await (
        backend as unknown as { _writeWrapperScript: () => Promise<void> }
      )._writeWrapperScript();
      const script = await readFile(join(sandboxHome, "lsfg"), "utf8");
      expect(script).toContain("export LSFG_LOG=1");
      expect(script).toContain("export VK_LOADER_DEBUG=layer");
    });

    it("verbose_logging off keeps the wrapper minimal", async () => {
      await backend.updateSettings({ verbose_logging: false });
      await (
        backend as unknown as { _writeWrapperScript: () => Promise<void> }
      )._writeWrapperScript();
      const script = await readFile(join(sandboxHome, "lsfg"), "utf8");
      expect(script).not.toContain("LSFG_LOG");
      expect(script).not.toContain("VK_LOADER_DEBUG");
    });
