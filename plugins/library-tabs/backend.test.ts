...
async function harness(
  ...
  const backend = new LibraryTabsBackend();
  ...
  }) as LibraryTabsBackend["callPlugin"];
...
describe("onLoad / getConfig", () => {
  it("seeds defaults on a first run", async () => {
    ...
    expect(config.schemaVersion).toBe(LIBRARY_TABS_SCHEMA_VERSION);
...
describe("setConfig", () => {
  ...
  it("survives a reload", async () => {
    ...
    const second = new LibraryTabsBackend();
    await second.onLoad();
...
describe("backups", () => {
  ...
  it("rejects a bogus backup filename with a user-facing message", async () => {
    ...
    await expect(backend.restoreBackupFile("../../etc/passwd")).rejects.toThrow(
      /isn't a Library Tabs backup file/,
    );
  });
});
...
describe("read-only mode", () => {
  ...
  async function futureBackend(): Promise<LibraryTabsBackend> {
    const seeder = new LibraryTabsBackend();
    await seeder.onLoad();
    const { writeFileSync, mkdirSync } = await import("node:fs");
    const { pluginStoragePath } = await import("@loadout/plugin-storage");
    const path = pluginStoragePath("library-tabs");
    mkdirSync(join(path, ".."), { recursive: true });
    writeFileSync(
      path,
      JSON.stringify({ ...defaultConfig(), schemaVersion: LIBRARY_TABS_SCHEMA_VERSION + 5 }),
    );
