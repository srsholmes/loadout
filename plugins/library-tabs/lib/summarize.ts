...
export function summarizeGrouping(spec: GroupSpec): string {
  if (spec.kind === "none") return "not grouped";
  if (spec.kind === "field") return `grouped by ${spec.field}`;
  return `${spec.groups.length} sub-tab${spec.groups.length === 1 ? "" : "s"}`;
}

/**
 * A whole tab as one sentence, for the tab list and for share previews.
 * e.g. `Show a game when (is installed and has never been played), sorted
 * by name ascending, first 30`.
 */
export function summarizeTab(tab: Tab): string {
  const parts: string[] = [];
  ...
  if (tab.group.kind !== "none") parts.push(summarizeGrouping(tab.group));
