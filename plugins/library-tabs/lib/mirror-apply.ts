...
export async function applyMirrorPlan(args: {
  ...
  const written = new Map<string, { collectionId: string; name: string; appIds: string[] }>();
  ...
  for (const del of plan.deletes) {
    try {
      await ops.remove({ collectionId: del.collectionId });
      deletedTabs.add(del.tabId);
      deleted++;
    } catch (err) {
      failures.push({ tabId: del.tabId, step: "delete", message: message(err) });
    }
  }
  ...
  for (const ren of plan.renames) {
    try {
      await ops.rename({ collectionId: ren.collectionId, name: ren.to });
      renamed++;
    } catch (err) {
      failures.push({ tabId: ren.tabId, step: "rename", message: message(err) });
    }
  }
  ...
  for (const create of plan.creates) {
    try {
      ...
      failures.push({ tabId: create.tabId, step: "create", message: message(err) });
    }
  }
  ...
  for (const update of plan.updates) {
    ...
    try {
      await ops.setApps({ collectionId: update.collectionId, appIds: update.appIds });
      written.set(update.tabId, {
      ...
      failures.push({ tabId: update.tabId, step: "update", message: message(err) });
    }
  }
  ...
  for (const ren of plan.renames) {
    if (written.has(ren.tabId)) continue;
    if (failures.some((f) => f.tabId === ren.tabId && f.step === "rename")) continue;
    const existing = ledger.entries.find((e) => e.tabId === ren.tabId);
    if (!existing) continue;
    written.set(ren.tabId, {
      collectionId: ren.collectionId,
      name: ren.to,
  ...
  const effective: MirrorPlan = {
    ...plan,
    deletes: plan.deletes.filter((d) => deletedTabs.has(d.tabId)),
  };
