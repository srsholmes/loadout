...
import { LIBRARY_TABS_SCHEMA_VERSION, uniqueTabId } from "./config";
import { migrate } from "./migrations";

/** Discriminator, so we can reject a code meant for something else. */
export const SHARE_KIND = "loadout.library-tabs";
 ...
 * a person pasted, so "that doesn't look like a Library Tabs code" is more
...
export function decodeShareString(text: string): ShareEnvelope {
  ...
  if (env.kind !== SHARE_KIND) {
    throw new Error("That code isn't a Library Tabs share code");
  }
