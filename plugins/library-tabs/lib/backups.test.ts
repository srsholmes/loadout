...
beforeEach(() => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), "library-tabs-backups-"));
...
function configWithTabs(labels: string[]): LibraryTabsConfig {
...
describe("backupDir", () => {
  it("sits next to the live config, not under a cache directory", () => {
    ...
    expect(backupDir()).toBe(
      join(tempDir, "loadout", "plugins", "library-tabs.backups"),
    );
...
describe("readBackup", () => {
  it("rejects a filename that isn't a backup", async () => {
    await expect(readBackup("../../etc/passwd")).rejects.toThrow(
      "That isn't a Library Tabs backup file",
    );
  });
  ...
  it("refuses a path-traversal attempt via the filename", async () => {
    ...
    ).rejects.toThrow("isn't a Library Tabs backup file");
  });
