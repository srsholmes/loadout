 ...
 *
 *   ~/.config/loadout/plugins/library-tabs.backups/
 *
...
export const PLUGIN_ID = "library-tabs";
 ...
 * needs to hear.
 */
export async function readBackup(file: string): Promise<LibraryTabsConfig> {
  if (parseBackupFilename(file) === null) {
    throw new Error("That isn't a Library Tabs backup file");
  }
  let raw: string;
  try {
