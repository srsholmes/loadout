...
beforeEach(() => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), "library-tabs-storage-"));
...
const CONFIG_PATH = () => pluginStoragePath("library-tabs");
...
function readStored(): LibraryTabsConfig {
  return JSON.parse(readFileSync(CONFIG_PATH(), "utf-8")) as LibraryTabsConfig;
}

function renamedTab(label: string): LibraryTabsConfig {
...
describe("loadConfig — a config from a newer build", () => {
  const future = () => ({
    ...defaultConfig(),
    schemaVersion: LIBRARY_TABS_SCHEMA_VERSION + 3,
  });
  ...
  it("does NOT overwrite the file", async () => {
    ...
    writeStored(future());
    await loadConfig();
    expect(readStored().schemaVersion).toBe(LIBRARY_TABS_SCHEMA_VERSION + 3);
  });
...
describe("loadConfig — daily backup", () => {
  ...
  it("prunes to the configured maximum", async () => {
    const config: LibraryTabsConfig = {
      ...defaultConfig(),
