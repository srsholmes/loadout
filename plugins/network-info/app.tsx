...
interface Measurement { bytes: number; bps: number; duration: number }
...
class SpeedTestEngine {
  ...
  onChartUpdate: (type: "download" | "upload", points: number[], p90: number) => void = () => {};
  ...
  async start() {
    ...
    const results: SpeedResults = {
      ...
      downloadLatency: null, downloadJitter: null,
      uploadLatency: null, uploadJitter: null, measuredAt: null,
    };
  ...
  private aborted() { return this.abortController?.signal.aborted ?? false; }
  ...
  private calcLatencyStats(latencies: number[]) {
    ...
    const jitter = latencies.reduce((s, p) => s + Math.abs(p - latency), 0) / latencies.length;
  ...
  private async measureDownload(results: SpeedResults) {
    const configs = [
      { bytes: 100000, count: 4 }, { bytes: 1000000, count: 4 },
      { bytes: 5000000, count: 3 }, { bytes: 10000000, count: 3 },
      { bytes: 25000000, count: 2 },
    ];
    return this.measureTransfer("download", configs, results, async (bytes) => {
      const res = await fetch(`https://speed.cloudflare.com/__down?bytes=${bytes}&r=${Math.random()}`, {
        signal: this.abortController?.signal, cache: "no-store",
      });
  ...
  private async measureUpload(results: SpeedResults) {
    const configs = [
      { bytes: 100000, count: 4 }, { bytes: 500000, count: 3 },
      { bytes: 1000000, count: 3 }, { bytes: 5000000, count: 2 },
    ];
    ...
    for (const config of configs) {
      ...
      for (let i = 0; i < config.count; i++) {
        ...
        try {
          ...
          this.onChartUpdate(type, measurements.map(m => m.bps), speed);

          if (duration > this.MAX_DURATION_MS) { shouldStop = true; break; }
        } catch { /* continue */ }
      }
    }
...
function useAnimatedValue(target: number | null, duration = 500): number | null {
  ...
  // Mirror `display` into a ref so the animation can read the latest
  // value when it (re)starts without listing `display` in the effect
  // deps — which would re-trigger on every animation tick and never
  // settle.
...
const fmtSpeed = (bps: number | null) => bps === null ? "--" : (bps / 1_000_000).toFixed(1);
const fmtLatency = (ms: number | null) => ms === null ? "--" : ms.toFixed(1);
const fmtTime = (d: Date | null) => d === null ? "--" : d.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", second: "2-digit" });

function signalLabel(signal: number | null): string {
...
function AnimatedNumber({ value, format }: { value: number | null; format: (v: number | null) => string }) {
...
function NetworkInfo() {
  ...
  useEffect(() => { fetchCloudflareLocation().then(setServer); }, []);
  ...
  const startTest = useCallback(() => {
    ...
    engine.onProgress = (r, p) => { setResults(prev => ({ ...prev, ...r })); setPhase(p); };
    engine.onComplete = (r) => { setResults(r); setStatus("finished"); setPhase("done"); };
  ...
  const phaseLabel = phase === "latency" ? "Testing latency…"
  ...
  const primaryIface = interfaces.find((i) => i.state === "up") ?? interfaces[0] ?? null;
        ...
        <div className="card">
          ...
          <div className="subsection">
            <div className="flex items-center gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mono text-[11.5px] text-base-content/50 truncate">{headerSub}</div>
              </div>
          ...
          <div className="subsection">
            <div className="flex items-center justify-between mb-2">
              ...
              {status === "idle" && (
                <Button variant="primary" onClick={startTest}>Run Speed Test</Button>
              )}
              {status === "running" && (
                <Button onClick={() => { engineRef.current?.stop(); setStatus("finished"); }}>Stop</Button>
              )}
              {(status === "finished" || status === "error") && (
                <Button variant="primary" onClick={startTest}>Retest</Button>
              )}
            </div>
            ...
            {status === "finished" && results.measuredAt && (
              <div className="subsection-desc mt-2 mono">
                Latency {fmtLatency(results.latency)} ms · Jitter {fmtLatency(results.jitter)} ms · Measured {fmtTime(results.measuredAt)}
              </div>
            )}
          </div>
