 ...
 * asks `collections.getSnapshot` over the loader's websocket — the same call
...
function profileMarkdown(meta: CorpusMeta, games: readonly GameMetadata[]): string {
  ...
  const lines: string[] = [
    ...
    "",
    "Generated by `plugins/collections/scripts/capture-library-corpus.ts`.",
...
async function main(): Promise<void> {
  ...
  process.stderr.write("Asking collections for its snapshot…\n");
  const snapshot = await callPlugin<GameMetadataSnapshot>("collections", "getSnapshot");
