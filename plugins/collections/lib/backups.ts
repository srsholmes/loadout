...
/**
 * How many backups to keep. A constant rather than a setting: nobody has ever
 * wanted to tune this, and a number on a settings screen invites someone to
 * set it to 1 the day before they need the other nineteen.
 */
...
export async function writeBackup(
  ...
  return {
    ...
    collectionLabels: config.collections.map((t) => t.label),
  };
}
...
export async function listBackups(): Promise<BackupInfo[]> {
  ...
  for (const file of files.sort().reverse()) {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as { collections?: Array<{ label?: unknown }> };
      const labels = Array.isArray(parsed.collections)
        ? parsed.collections
            .map((t) => (typeof t?.label === "string" ? t.label : null))
            .filter((l): l is string => l !== null)
        : [];
      out.push({ file, ...meta, collectionCount: labels.length, collectionLabels: labels });
    } catch {
      // Unreadable: still list it, so the user can see it exists and that
      // it isn't a candidate, rather than wondering where it went.
      out.push({ file, ...meta, collectionCount: 0, collectionLabels: [] });
    }
  }
  return out;
}
