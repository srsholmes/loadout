...
export function validateConfig(raw: unknown): string[] {
  ...
  } else if (raw.schemaVersion > COLLECTIONS_SCHEMA_VERSION) {
    problems.push(
 ...
 */
export function orderedTabs(config: CollectionsConfig): Tab[] {
...
export function resolveDefaultTab(config: CollectionsConfig): Tab | null {
...
export function uniqueTabId(config: CollectionsConfig, base: string): string {
