...
export type { Verdict, EvalGroupResult };
 ...
 */
function decide(verdict: Verdict, policy: Tab["indeterminatePolicy"]): boolean {
...
export interface EvalResult {
  ...
  /** Post-sort, post-cap, post-group. One synthetic group when ungrouped. */
...
function fillWithoutThis(
  ...
  policy: Tab["indeterminatePolicy"],
  node: RuleNodeTrace,
): void {
...
export function countMatches(
  tab: Pick<Tab, "root" | "indeterminatePolicy">,
  games: readonly EvalGame[],
): number {
  let n = 0;
  for (const game of games) {
