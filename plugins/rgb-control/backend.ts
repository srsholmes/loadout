...
const ALL_MODES = ["static", "breathing", "rainbow", "off"] as const;

// ── OXP HID V2 Protocol ─────────────────────────────────────────
// OneXPlayer Apex RGB controller: VID 0x1A2C, PID 0xB001
// Uses 64-byte HID output reports with V2 framing: [cid, 0xFF, ...payload]

const OXP_VID = "1A2C";
const OXP_PID = "B001";

/** OXP effect presets sent via command ID 0x07 */
const OXP_EFFECTS: Record<string, number> = {
  aurora: 0x01,
  flowing: 0x03,
  neon: 0x05,
  dreamy: 0x07,
...
const OXP_MODES = ["static", "off", ...Object.keys(OXP_EFFECTS)] as const;

/** Build a 64-byte V2 HID command */
function oxpCmd(cid: number, payload: number[]): Buffer {
...
async function commandExists(cmd: string): Promise<boolean> {
  const result = await exec(`which ${cmd} 2>/dev/null`);
...
export default class RgbControlBackend implements PluginBackend {
  ...
  async setColor(zone: string, r: number, g: number, b: number): Promise<boolean> {
    ...
    if (z.id.startsWith("oxp:")) {
      ...
      const level: "low" | "medium" | "high" =
        targetPct <= 33 ? "low" : targetPct <= 66 ? "medium" : "high";
  ...
  async setBrightness(zone: string, percent: number): Promise<boolean> {
    ...
    if (z.id.startsWith("oxp:")) {
      ...
      if (percent <= 33) level = "low";
      else if (percent <= 66) level = "medium";
  ...
  private async oxpSetBrightnessLevel(
    ...
    const bc = level === "low" ? 0x01 : level === "medium" ? 0x03 : 0x04;
