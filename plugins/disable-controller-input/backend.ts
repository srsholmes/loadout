...
/** Parse the value half of `busctl get-property` output. Format is
 *  `<sig> <value...>` where the value rendering depends on the
 *  signature. We handle the two shapes this plugin actually reads:
 *    s "string"
 *    ao 2 "/path1" "/path2"
 *
 *  Anything more exotic is returned as the raw remainder so the caller
 *  can decide what to do.
 */
function parseStringProp(stdout: string): string | null {
  // Expected: `s "value"\n`. Strip leading sig token.
  const trimmed = stdout.trim();
  const m = trimmed.match(/^s\s+"((?:\\.|[^"\\])*)"$/);
  if (!m) return null;
  // Unescape \" and \\ — the rest of the busctl-quoted set we don't
  // care about (no embedded newlines in InputPlumber names).
  return m[1].replace(/\\"/g, '"').replace(/\\\\/g, "\\");
}

function parseObjectPathArrayProp(stdout: string): string[] | null {
  const trimmed = stdout.trim();
  // `ao N "/p1" "/p2" ...`  — N is the count, then quoted paths.
...
async function listCompositeDevicePaths(): Promise<string[]> {
  ...
  for (const line of r.stdout.split("\n")) {
    const t = line.trim();
    // Match /org/shadowblip/InputPlumber/CompositeDevice<digits> exactly,
    // ignoring child paths and the Manager / devices subtrees.
...
export default class DisableControllerInputBackend implements PluginBackend {
  ...
  async refreshControllers(): Promise<ListResult> {
    if (!this.unavailable) await this.reconcile();
