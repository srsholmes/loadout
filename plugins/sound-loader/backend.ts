...
const SOUND_PACKS_DIR = join(homedir(), ".local/share/loadout/sound-packs");
const CONFIG_PATH = join(homedir(), ".local/share/loadout/sound-loader.json");
const VALID_EXTENSIONS = new Set([".wav", ".mp3", ".ogg"]);

/**
 * Sound Loader plugin backend.
 *
 * Discovers sound packs from ~/.local/share/loadout/sound-packs/,
 * serves audio file data to the frontend, and persists the active pack selection.
 *
  ...
  private async _loadConfig(): Promise<void> {
    try {
      ...
      if (await file.exists()) {
        const loaded = (await file.json()) as Partial<SoundLoaderConfig>;
        this.config = {
