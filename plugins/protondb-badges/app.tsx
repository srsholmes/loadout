...
/**
 * Mirror of `GameInfo` from the game-browser plugin's backend. We
 * consume `getGames` directly via cross-plugin RPC so collection tags
 * and local-art URLs are available on every tile.
 */
...
function ProtonDBBadges() {
  ...
  // Library data + collection tags come from the game-browser plugin
  // (single source of truth across SGDB / LSFG-VK / HLTB / here), not
  // from steam-paths' bare `listInstalledGames`.
  ...
  useEffect(() => {
    ...
    // ProtonDB only rates Steam-released games — drop shortcuts here
    // so the picker doesn't show 314 emulator titles that can never
    // get a tier.
    void gameBrowser
      .call("getGames")
      .then((games) => {
        const list = Array.isArray(games) ? (games as InstalledGame[]) : [];
        setInstalled(list.filter((g) => g.source === "steam"));
      })
      .catch(() => setInstalled([]));
  }, [call, gameBrowser]);
  ...
  useEvent({
    ...
    handler: (data: any) => {
      if (data.connected !== undefined) {
        setStatus({ connected: data.connected, tabs: data.tabs ?? 0 });
      }
      if (data.settings) {
  ...
  const tierCounts = useMemo<Record<TierKey, number>>(() => {
    const counts: Record<TierKey, number> = {
      platinum: 0, gold: 0, silver: 0, bronze: 0, borked: 0,
    };
...
function ProtonDBGameCard({
  ...
  // so we stay in sync with what SGDB shows; the local endpoint is
  // only swapped in by SGDB itself after applying custom art.
  ...
  // Same pattern as HLTB: every fetch state lives in the bottom-of-
  // image overlay slot as a pill. Keeps every ProtonDB tile the same
  // height regardless of which cards have resolved their tier yet
  // (background fetches are rate-limited, so cards trickle in).
  // The final fallback covers tiers that aren't in `TIER_ORDER`
  // (mainly "pending" — community reports exist but no rating yet) —
  // without it, those tiles would render a blank overlay and look
  // visually broken next to other tiles that have a chip.
...
// --- Home widget (unchanged behaviour) ---
 ...
 * the search input, gear/back toggle, and the running-game pointer
 * share the body's React tree without any cross-root pub/sub.
 */
