...
export default class FanControlBackend implements PluginBackend {
  ...
  private async scanHwmonDevices(): Promise<HwmonDevice[]> {
    ...
    try {
      const entries = await readdir(HWMON_BASE);
      ...
      for (const entry of entries) {
        ...
        for (let i = 1; i <= 8; i++) {
          ...
          const pwmPath = existsSync(`${dir}/pwm${i}`) ? `${dir}/pwm${i}` : null;
          const pwmEnablePath = existsSync(`${dir}/pwm${i}_enable`) ? `${dir}/pwm${i}_enable` : null;
  ...
  private async saveOriginalModes(): Promise<void> {
    ...
    for (const fan of this.activeFanDevice.fans) {
      if (fan.pwmEnablePath) {
        try {
          const val = await readFile(fan.pwmEnablePath, "utf-8");
