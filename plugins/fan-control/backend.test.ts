...
// fs mocking strategy: `spyOn` the live namespace bindings, NOT
// `mock.module("fs", …)`. In this repo `bun run test` discovers every
// *.test.ts into ONE process and there is no per-spec fork, so a
// top-level `mock.module("fs"/"fs/promises", …)` would leak into the
// loader specs that import `node:fs/promises` (Bun normalises the
// `node:` prefix, so the mock spans both). `spyOn(...).mockRestore()`
// patches the live binding and cleans up per-test — see
// docs/test-mock-contamination.md convention #1. backend.ts uses
// namespace imports (`fsp.readFile`, `fs.existsSync`) so the spies land
...
// so every existing callsite keeps working unchanged.
const mockReaddir = mock((..._args: unknown[]) => Promise.resolve([] as string[]));
const mockReadFile = mock((..._args: unknown[]) => Promise.resolve(""));
...
describe("FanControlBackend", () => {
  ...
  beforeEach(() => {
    ...
    // Reset the delegate mocks to their defaults (mockClear only wipes
    // call history, not an implementation a prior test installed).
    mockReaddir.mockReset();
    mockReadFile.mockReset();
    mockExistsSync.mockReset();
    mockReaddir.mockImplementation(() => Promise.resolve([] as string[]));
    mockReadFile.mockImplementation(() => Promise.resolve(""));
    mockExistsSync.mockImplementation(() => false);

    // Spy the live fs namespaces (no mock.module — see header note).
    readdirSpy = spyOn(fsp, "readdir").mockImplementation(
      ((...a: unknown[]) => mockReaddir(...a)) as typeof fsp.readdir,
    );
    readFileSpy = spyOn(fsp, "readFile").mockImplementation(
      ((...a: unknown[]) => mockReadFile(...a)) as typeof fsp.readFile,
    );
    existsSyncSpy = spyOn(fs, "existsSync").mockImplementation(
      ((...a: unknown[]) => mockExistsSync(...a)) as typeof fs.existsSync,
    );
