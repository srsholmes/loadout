...
#
# Usage: sudo bash scripts/install-inputplumber.sh
#
...
# ── 1a. Conflict: HHD ───────────────────────────────────────────────
#
# Handheld Daemon and InputPlumber both want to own the controller's HID
# interface — running both means one wins the grab and the other sits
# inert (or worse, they fight over it and inputs drop). The upstream IP
# COPR even declares Conflicts: hhd. Stop+disable+mask any hhd*.service
# units before bringing IP up. Mirrors the pattern in apex-fixes'
# migrate-to-inputplumber.sh.
HHD_UNITS=()
...
if [ ${#HHD_UNITS[@]} -gt 0 ]; then
    log "── disabling conflicting Handheld Daemon (HHD) units ──"
    for unit in "${HHD_UNITS[@]}"; do
        log "stopping + masking $unit"
...
# ── 1b. Already installed? Just make sure the service is up ─────────
if detect_installed; then
    bin="$(command -v inputplumber 2>/dev/null || echo "$IP_BIN_DST")"
    log "InputPlumber already installed at $bin"
    systemctl daemon-reload
    if ! systemctl is-enabled --quiet inputplumber.service 2>/dev/null; then
        log "enabling inputplumber.service"
        systemctl enable inputplumber.service
    fi
    if ! systemctl is-active --quiet inputplumber.service; then
        log "starting inputplumber.service"
        systemctl start inputplumber.service
    else
        log "inputplumber.service already active"
    fi
...
[Unit]
Description=InputPlumber — input routing daemon (loadout install)
After=dbus.service
Wants=dbus.service
