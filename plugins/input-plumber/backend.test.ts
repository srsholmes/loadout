 ...
 *
 * The backend is a thin orchestrator over `./lib/install`:
 *   - it polls `getStatus()` every 30s and re-emits it as `input-plumber-status`
 *   - it serialises `install()` runs and frames them with
 *     `install-state` + `install-log` events
 *
 * Rather than reach through to real subprocesses, we mock the install
 * module wholesale and assert on the events the backend emits plus the
 * call shape it makes into the installer.
 */
...
describe("InputPlumberBackend", () => {
  ...
  afterEach(async () => {
    // Always tear down the status timer to keep `bun test` from
