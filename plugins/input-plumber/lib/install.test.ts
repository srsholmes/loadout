/**
 * InputPlumber install module — integration tests.
 *
 * Tests the install probe (getStatus dispatches `which inputplumber`,
 * `inputplumber --version`, `systemctl is-active/is-enabled` via
 * @loadout/exec) and the install cancellation path (the cancellation
 * handle wired into `install()` sends SIGTERM to the running script).
 *
 * Both code paths shell out via Bun.spawn (through @loadout/exec), so
 * we swap Bun.spawn for the duration of the suite and dispatch on the
 * argv to fake `which` / `systemctl` / install-script output.
 */
...
// @loadout/exec's runFull/spawn both call Bun.spawn under the hood.
// We provide a small process double that exposes stdout/stderr
...
function makeProc(opts: ProcOptions = {}): {
  ...
  return {
    ...
    kill: (sig?: string | number) => {
      ...
      // The streaming install path expects the child to die after kill();
...
// Import the SUT after the Bun.spawn shim is in place. (`@loadout/exec`'s
// `spawn` is a function — not a captured const — so re-assignment is honoured.)
...
/**
 * Dispatch spawn calls by inspecting argv. argv is `[cmd, ...args]`
 * directly (no sudo prefix in the loadout port).
 */
function routeSpawn(
  ...
  spawnImpl = (argv: string[]) => {
    ...
    // If router returned a full proc (has `.kill`), pass through.
...
describe("InputPlumberBackend.cancelInstall — cancellation path", () => {
  ...
  it("kills the install child via SIGTERM and resolves the awaiting caller", async () => {
    ...
    // mark it present so the spawn actually fires. install.ts resolves
    // the script path as join(import.meta.dir, "..", "scripts", …) from
    // inside lib/install.ts, which lands at the plugin's scripts dir.
    ...
    // The kill() handle wired into install() should have fired with
    // SIGTERM (the streaming variant uses SIGTERM; the timeout path
    // uses SIGKILL).
