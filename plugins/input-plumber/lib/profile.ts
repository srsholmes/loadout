...
/** Where the Deck hidraw uaccess rule installs. Number above 70 so it
 *  applies AFTER systemd's own uaccess.rules; below 90 so distros' own
 *  later overrides still win. */
export const DECK_HIDRAW_UACCESS_RULE_PATH =
  "/etc/udev/rules.d/72-loadout-deck-hidraw.rules";

/** udev rule granting the active-session user `rw` ACL on the Steam Deck's
 *  HID gamepad interface so the overlay (running as the deck user) can read
 *  `/dev/hidraw{0,1,2}` in parallel with Steam Input.
 *
 *  SteamOS ships an equivalent rule via the `steam-jupiter-stable` package
 *  (60-steam-input.rules), so on SteamOS this is a redundant no-op. On
 *  Bazzite-Deck / vanilla-Arch-Deck / any non-SteamOS-but-on-Deck install
 *  where Steam's package may not be present (or hasn't been installed yet
 *  by the user), this is what makes the wake button work out of the box.
 *
 *  Scoped to Valve VID 0x28DE products 0x1205 (Jupiter / LCD) and 0x1206
 *  (Galileo / OLED) to keep the blast radius narrow — we don't grant
 *  access to other Valve hardware (Index controllers, external Steam
 *  Controllers) the user might have plugged in. */
export const DECK_HIDRAW_UACCESS_RULE =
  `# Generated by Loadout — grant the active-session user rw ACL on the\n` +
  `# Steam Deck's hidraw nodes so the overlay can read button state in\n` +
  `# parallel with Steam Input. Idempotent with Steam's own rule from\n` +
  `# /usr/lib/udev/rules.d/60-steam-input.rules where that ships.\n` +
  `KERNEL=="hidraw*", SUBSYSTEM=="hidraw", ATTRS{idVendor}=="28de", ATTRS{idProduct}=="1205", MODE="0660", TAG+="uaccess"\n` +
  `KERNEL=="hidraw*", SUBSYSTEM=="hidraw", ATTRS{idVendor}=="28de", ATTRS{idProduct}=="1206", MODE="0660", TAG+="uaccess"\n`;
