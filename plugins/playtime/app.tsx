...
function formatHoursNumber(ms: number): number {
  return ms / 3_600_000;
}

function formatHoursStr(ms: number, digits = 1): string {
...
function PlayTime() {
  ...
  const period = stats ? stats[range] : null;
  ...
  const bars = stats?.weeklyBreakdown ?? [];
  const maxBarMs = Math.max(1, ...bars.map((b) => b.totalMs));
  const lastBarIdx = bars.length - 1;

  // Aggregate sessions (approx) — we don't have session-count, so derive from
  // game count as a conservative display; skip longest/streak (no data).
  ...
  // period segmented (Day / Week / Month / All) on the right. Same
  // React tree as the body — `range` state is shared by closure.
  const headerNode = (
    <PluginHeader>
      ...
      <div className="page-content">
        <div className="card">
          ...
          {/* HEADER: total hours + 7-day bars. Period selector lives
              in the portaled topbar header now, so the body just
              shows the headline metric for the active period. */}
          ...
          <div className="subsection">
            ...
            >
              ...
              <InsetStat
                ...
                value={
                  topGames[0]
                    ? formatHoursStr(topGames[0].totalMs, 1)
                    : "0"
                }
                unit="h"
                tone="var(--accent)"
