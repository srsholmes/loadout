...
function BatteryTracker() {
  ...
  useEffect(() => {
    call("getBatteryInfo").then((info) => {
      const data = info as BatteryInfo;
      ...
      <div className="page-content">
        ...
        <div className="card">
          <div className="card-header flex items-center justify-between py-3.5 px-4.5 border-b border-base-300">
            ...
            {charging ? (
              <span className="chip chip-success"><FaBolt className="w-2.5 h-2.5" /> CHARGING</span>
            ) : (
          ...
          <div className="px-6 py-7">
            <div className="flex items-baseline justify-center gap-2 mb-5">
              ...
              <span className="metric-unit" style={{ fontSize: 18 }}>%</span>
            </div>
        ...
        <div className="card">
          <div className="subsection">
            ...
            <div className="grid grid-cols-3 gap-2.5 mb-3">
              <MetricTile label="WATTS" value={`${drawW >= 0 ? "+" : ""}${drawW.toFixed(1)}`} color={drawW >= 0 ? "var(--color-success)" : "var(--fg-1)"} />
          ...
          <div className="subsection">
            <div className="flex items-center justify-between mb-3.5">
              ...
              <span className="chip" style={{ borderColor: segColor, color: segColor }}>● {healthPct}%</span>
            </div>
            <div className="grid grid-cols-2 gap-2.5 mb-3.5">
              <MetricTile label="CAPACITY" value={`${battery.energyFullWh.toFixed(1)}`} unit=" Wh" />
          ...
          <div className="subsection">
            ...
            <div className="row"><span className="row-label">Design capacity</span>     <span className="row-value">{battery.energyFullDesignWh.toFixed(1)} Wh</span></div>
            <div className="row"><span className="row-label">Full-charge capacity</span><span className="row-value">{battery.energyFullWh.toFixed(1)} Wh</span></div>
            <div className="row"><span className="row-label">Current charge</span>      <span className="row-value">{battery.energyNowWh.toFixed(1)} Wh</span></div>
            <div className="row"><span className="row-label">Voltage</span>             <span className="row-value">{voltage.toFixed(2)} V</span></div>
            <div className="row"><span className="row-label">Status</span>              <span className="row-value">{battery.status}</span></div>
            <div className="row"><span className="row-label">Time remaining</span>      <span className="row-value">{battery.timeRemainingFormatted}</span></div>
          </div>
...
export function mount(container: HTMLElement, opts?: { parentFocusKey?: string }): () => void {
...
/** Homepage widget — compact battery display for dashboard. */
