...
function floatToLong(x: number): number {
  const buf = new ArrayBuffer(4);
  new Float32Array(buf)[0] = x;
  return new Uint32Array(buf)[0];
}

function longToFloat(x: number): number {
  const buf = new ArrayBuffer(4);
...
/** Convert color temperature in Kelvin to RGB gamma multipliers (Tanner Helland algorithm). */
function kelvinToGamma(kelvin: number): { r: number; g: number; b: number } {
...
export default class DisplaySettingsBackend implements PluginBackend {
  ...
  private async detectMethod(): Promise<void> {
  ...
  private async readCurrentState(): Promise<void> {
    ...
    if (this.backlightPath && this.maxBrightness > 0) {
      try {
        ...
        this.state.brightness = Math.round((current / this.maxBrightness) * 100);
      } catch {
        // ignore
      }
    }
  ...
  async setBrightness(value: number): Promise<boolean> {
    ...
    if (this.backlightPath && this.backlightName && this.maxBrightness > 0) {
      const rawValue = Math.round((value / 100) * this.maxBrightness);
