...
function DisplaySettings() {
  ...
  const handleSaturation = useCallback((v: number) => { setSaturation(v); setActivePreset(null); call("setSaturation", v); }, [call]);
  const handleBrightness = useCallback((v: number) => { setBrightness(v); setActivePreset(null); call("setBrightness", v); }, [call]);
  const handleColorTemp = useCallback((v: number) => { setColorTemp(v); setActivePreset(null); call("setColorTemp", v); }, [call]);
  const handlePreset = useCallback((name: string) => { setActivePreset(name); call("applyPreset", name); }, [call]);
  const handleReset = useCallback(() => { setActivePreset("default"); call("resetDefaults"); }, [call]);

  const methodLabel =
    info?.method === "gamescope" ? "Gamescope"
    : info?.method === "xrandr" ? `xrandr (${info.xrandrOutput})`
      ...
      <div className="page-content">
        <div className="card">
          ...
          <div className="subsection">
            ...
            <Slider value={brightness} min={0} max={100} step={1} onChange={handleBrightness} />
          ...
          <div className="subsection">
            ...
            <Slider value={saturation} min={0} max={200} step={1} onChange={handleSaturation} />
            <div className="flex justify-between mono text-[11px] text-base-content/50 mt-1.5">
          ...
          <div className="subsection">
            ...
            <Slider value={colorTemp} min={3000} max={6500} step={100} onChange={handleColorTemp} />
            <div className="flex justify-between mono text-[11px] text-base-content/50 mt-1.5">
              <span>Warm · 3000K</span><span>Neutral</span><span>Cool · 6500K</span>
            </div>
            {/* Warm quick-picks */}
          ...
          <div className="subsection">
            ...
            <div className="row"><span className="row-label">Red</span>   <span className="row-value">{gamma.r.toFixed(2)}</span></div>
            <div className="row"><span className="row-label">Green</span> <span className="row-value">{gamma.g.toFixed(2)}</span></div>
            <div className="row"><span className="row-label">Blue</span>  <span className="row-value">{gamma.b.toFixed(2)}</span></div>
          </div>

          {/* Info + Reset */}
          <div className="subsection">
            <div className="subsection-label">Display</div>
            <div className="row"><span className="row-label">Control method</span><span className="row-value">{methodLabel}</span></div>
            {info?.backlightPath && (
              <div className="row"><span className="row-label">Backlight</span><span className="row-value">{info.backlightPath}</span></div>
            )}
            <div className="flex justify-between items-center pt-3.5 mt-3.5 border-t border-base-300">
              <div className="subsection-desc m-0">
                Changes apply live. Adjustments vary with display hardware + compositor.
              </div>
...
function DisplayHomeWidget() {
  ...
  useEffect(() => {
    call("getState").then((result) => {
      const state = result as DisplayState;
      setBrightness(state.brightness);
    }).catch(() => {});
    call("getPresets").then((result) => setPresets(result as Preset[])).catch(() => {});
  }, [call]);
  ...
  return (
    <div className="p-4">
      <div className="flex items-baseline gap-2 mb-3">
        <span className="metric-value mono" style={{ fontSize: 36 }}>{brightness}</span>
      ...
      <Slider
        ...
        onChange={(val) => {
          ...
          debounceRef.current = setTimeout(() => { call("setBrightness", val).catch(() => {}); }, 500);
        }}
      />
      {presets.length > 0 && (
