...
type GameSource = "steam" | "shortcut";

/**
 * Shape we consume from the `game-browser` plugin's `getGames` RPC.
 * Mirrors the `GameInfo` interface in `plugins/game-browser/backend.ts`
 * — repeated here so the SGDB plugin doesn't take a cross-plugin type
 * dependency.
 */
interface GameInfo {
  appId: string;
  ...
  headerUrl: string;
  capsuleUrl: string;
  /** Forced local-endpoint URLs. Optional for backwards-compat with
   *  any old game-browser bundle still on disk; new builds always
   *  populate them, but we don't want a missing field to crash the
   *  picker. */
...
// `game-browser::getCollections` — same convention as LSFG-VK so the
// shape of the dropdown matches across plugins.
const ALL_GAMES = "__all__";
const STEAM_ONLY = "__steam__";
const SHORTCUT_ONLY = "__shortcut__";
type LibraryFilter = string;

/** Counted collection entry from `game-browser::getCollections`. */
...
function SteamGridDB() {
  ...
  // sourced from the game-browser plugin. Filtered client-side; the
  // library is small enough that pushing search to a backend would
  // add latency without measurable benefit.
  const [library, setLibrary] = useState<GameInfo[]>([]);
  /** Counted collections from `game-browser::getCollections`. Used to
   *  populate the filter dropdown — same shape as LSFG-VK. */
   ...
   *  `game-browser` RPC failed (most commonly: the plugin is disabled). */
  ...
  // Pull the unified library + collection counts from game-browser
  // once the API key is in place. Filter client-side: the library is
  // small enough (hundreds of entries) that pushing search to a
  // backend would add latency without measurable benefit. After a
  // successful load we ask our backend to prune saved SGDB matches
  // whose appId isn't in the live library anymore (deleted shortcuts)
  // so plugin storage doesn't grow without bound.
  useEffect(() => {
    ...
    void (async () => {
      try {
        const [games, cols] = await Promise.all([
          gameBrowser.call("getGames") as Promise<GameInfo[]>,
          gameBrowser
            .call("getCollections")
            .catch(() => []) as Promise<CollectionEntry[]>,
        ]);
        if (cancelled) return;
        ...
        console.warn("[steamgriddb] game-browser getGames failed:", err);
        if (cancelled) return;
            ...
            <div className="subsection">
              ...
              {typeof libraryStatus === "string" && libraryStatus !== "ok" && (
                <div className="subsection-desc mb-0">
                  Couldn't load the library from the{" "}
                  <span className="font-semibold">game-browser</span> plugin.
                  Make sure it's enabled in Settings, then reopen this
                  plugin. (Underlying error: {libraryStatus})
                </div>
              )}
 ...
 * content is now portaled from inside `mount()` via
 * `<PluginHeader>` in @loadout/ui — same React tree as the
 * body, so state and callbacks are shared without any cross-root
 * pub/sub. Implementation is intentionally empty.
 */
