...
// Also import the module-level parseDeviceType function indirectly via tests
// (it's used internally by getDevices)
...
describe("BluetoothBackend", () => {
  ...
  beforeEach(() => {
    ...
    // Stop poll interval to prevent timer leaks
  ...
  // ---------------------------------------------------------------------------
  // Device type classification (via getDevices)
  // ---------------------------------------------------------------------------

  describe("device type classification", () => {
    function setupInfoForIcon(icon: string) {
      mockRun.mockImplementation((cmd: string[]) => {
        if (cmd.includes("devices")) {
          return Promise.resolve({
            stdout: "Device 11:22:33:44:55:66 TestDev\n",
          });
        }
        if (cmd.includes("info")) {
          return Promise.resolve({
    ...
    it("classifies audio-headset as audio", async () => {
      setupInfoForIcon("audio-headset");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("audio");
    });

    it("classifies audio-headphone as audio", async () => {
      setupInfoForIcon("audio-headphone");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("audio");
    });

    it("classifies input-gaming as input", async () => {
      setupInfoForIcon("input-gaming");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("input");
    });

    it("classifies joystick as input", async () => {
      setupInfoForIcon("joystick");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("input");
    });

    it("classifies gamepad as input", async () => {
      setupInfoForIcon("gamepad");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("input");
    });

    it("classifies input-keyboard as keyboard", async () => {
      setupInfoForIcon("input-keyboard");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("keyboard");
    });

    it("classifies unknown icon as unknown", async () => {
      setupInfoForIcon("phone");
      const devices = await backend.getDevices();
      expect(devices[0].type).toBe("unknown");
    });

    it("returns unknown when no Icon field present", async () => {
      mockRun.mockImplementation((cmd: string[]) => {
        if (cmd.includes("devices")) {
          return Promise.resolve({
            stdout: "Device 11:22:33:44:55:66 TestDev\n",
  ...
  describe("connectDevice()", () => {
    it("updates the cache to connected=true", async () => {
      ...
      expect((backend as any).lastDeviceState.get("AA:BB:CC:DD:EE:FF")).toBe(
        true,
      );
    });
  });
