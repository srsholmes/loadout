...
async function readAllShortcutLaunchOptions(): Promise<Map<string, string>> {
  ...
  for (const userId of userIds) {
    ...
    for (const sc of Object.values(shortcuts)) {
      ...
      const s = sc as Record<string, any>;
 ...
 */
function getAppsSection(vdf: any): any | null {
  return (
    vdf?.UserLocalConfigStore?.Software?.Valve?.Steam?.apps ??
    vdf?.UserLocalConfigStore?.Software?.Valve?.Steam?.Apps ??
    null
  );
}
      ...
      try {
        ...
        for (const appId of Object.keys(apps)) {
          ...
          if (
            ...
            appData !== null &&
            appData.LaunchOptions
          ) {
            games.push({
              appId,
              launchOptions: appData.LaunchOptions,
            });
            seen.add(appId);
          }
        }
  ...
  async getLaunchOptions(appId: string): Promise<string> {
    ...
    for (const configPath of configs) {
      try {
        ...
        const lo = apps[appId].LaunchOptions;
