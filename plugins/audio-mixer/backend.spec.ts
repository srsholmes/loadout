...
// In-memory "filesystem" used by the plugin-storage helper. Mocking
// `fs/promises` (and Bun.write below) keeps tests hermetic from the
// real ~/.config/loadout directory.
...
const mockRename = mock(async (from: string, to: string) => {
  const val = files.get(from);
  if (val === undefined) throw new Error(`ENOENT: ${from}`);
  files.set(to, val);
  files.delete(from);
});
...
const bunWriteSpy = spyOn(Bun, "write").mockImplementation(
...
import AudioMixerBackend, { mixerChanged, type MixerState } from "./backend";
...
describe("AudioMixerBackend", () => {
  ...
  function mountSpawn(handler: (argv: string[]) => FakeRun | null) {
    spawnSpy.mockImplementation((cmd: any) =>
      spawnRouter([handler])(cmd) as any,
    );
  }
  ...
  describe("availability", () => {
    it("marks unavailable when wpctl is missing", async () => {
      mountSpawn((argv) => {
        if (argv[0] === "which" && argv[1] === "wpctl")
          return { exitCode: 1 };
        if (argv[0] === "which" && argv[1] === "pw-dump")
          return { exitCode: 0 };
        return null;
      });
  ...
  describe("getMixerState (happy path)", () => {
    beforeEach(() => {
      mountSpawn((argv) => {
        if (argv[0] === "which") return { exitCode: 0 };
        if (argv[0] === "pw-dump") {
  ...
  describe("setVolume / setMute / setDefault", () => {
    ...
    it("returns failure when not available", async () => {
      mountSpawn((argv) => {
        if (argv[0] === "which" && argv[1] === "wpctl") return { exitCode: 1 };
        return { exitCode: 0 };
      });
      await backend.onLoad();
...
describe("mixerChanged", () => {
  ...
  it("returns false for identical state", () => {
    ...
    expect(mixerChanged(baseState, { ...baseState, sinks: [...baseState.sinks] })).toBe(
      false,
    );
  });
