 ...
 * Ignores non-metric keys (position, fps_limit, background_alpha, no_display).
 */
function detectActivePreset(
  config: Record<string, string>,
  presets: Preset[],
): string | null {
  const IGNORED = new Set(["position", "fps_limit", "background_alpha"]);
  const configKeys = Object.keys(config).filter((k) => !IGNORED.has(k));
...
function alphaToPercent(raw: string | undefined): number {
  if (!raw) return 50;
...
function percentToAlpha(pct: number): string {
