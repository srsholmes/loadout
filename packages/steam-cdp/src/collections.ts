...
export async function deleteCollection(
  ...
  const expr = `(async () => {
    ...
    try {
      ...
      // \`DeleteCollection\` takes the **id**, not the collection: its first act
      // is \`BIsSystemCollectionId(e)\`, which calls \`e.startsWith\`. Passing the
      // object threw "e.startsWith is not a function" — caught only by running
      // it against real Steam, because the test fake accepted either.
      const id = String(col.id != null ? col.id : col.m_strId);
      if (typeof cs.DeleteCollection === "function") await cs.DeleteCollection(id);
