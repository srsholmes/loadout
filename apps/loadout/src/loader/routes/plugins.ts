...
export const pluginI18nRoute: RouteHandler = {
  ...
  async handle(_req, url, ctx) {
    ...
    // Both groups are non-optional in I18N_PATTERN, but under
    // noUncheckedIndexedAccess they're `string | undefined` — narrow rather
    // than assert, so a future pattern edit fails loudly here.
    if (!match?.[1] || !match[2]) {
      return new Response("Not Found", { status: 404 });
    }
    const pluginId = match[1];
